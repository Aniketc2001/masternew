...
export default function SelectBoxDropdown({ dataSource, setpropName,setpropId,baseObj, value, initialText, initialId, ancobjectName, setbaseObj, data, dynamic,apiName,listType,fieldName, ancchild,setancds }) {
    ...
    const [selectboxValue, setselectboxValue] = useState(initialText)
    ...
    //Devextreme Autocomplete functions
    const handleInputChange = (e) => {
        //setSearchTerm(e.value);
        console.log('searchterm value',e);
        try{
            if (e.value.length === 3) {
                // Fetch data from API based on the search term
                fetchApiData(e.value).then((data) => {
                    console.log('fetchapi got data...',data);
                    setselectboxDatasource(data);
                });
            } 
        }
        catch(ex){}
    ...
    const fetchApiData = (searchTerm) => {
        // Replace with your API call to fetch the data based on the search term
        // Example: return fetch(`/api/data?search=${searchTerm}`).then(response => response.json());
        // Return a Promise that resolves to the JSON array
        // Simulating a delay with setTimeout in this example
        return new Promise((resolve) => {
          setTimeout(() => {
            var data;
            axios({
                method: 'get',
                url: apiName + '/' + listType + '/' + fieldName + '/' + searchTerm,
              }).then((response) => {
                console.log('getting data...',response.data.anc_results);
                data=response.data.anc_results;
    ...
    const handleSelectionChanged = (e) => {
        ...
        try{
            setpropName(e.selectedItem[data.displayExpr]);
            setpropId(e.selectedItem[data.valueExpr]);
        }
        catch(ex){}
    }

    //Devextreme SelectBox functions
    ...
    const handleKeyDown = (event) =>{
        try{
            ...
            console.log("keycode",event,event.event.keyCode);
            console.log('options',selectboxRef.current.instance.option());
            console.log('option text',selectboxRef.current.instance.option('text'));


            if(event.event.keyCode === 13){
                console.log("enter pressed...",apiName,listType,fieldName,s);
                setcurrentSBText(s);
    ...
    return (
        <>
         {dynamic ?
                    ...
                    //     valueExpr={data.displayExpr}
                    //     searchMode="contains"
                    //     value={initialText}
                    //     labelMode='floating'
                    //     placeholder='Type atleast 31 chars to search...'
                    //     stylingMode="filled"
                    //     label={data.label}
