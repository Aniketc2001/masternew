...
export default function DataFilter() {
    ...
    const validateForm = () => {
        ...
        const filteredConditionData = baseObj.DataFilterConditions.filter(row => row.ConditionOperatorId != null && row.FilterClause === "");
        if (filteredConditionData.length > 0) {
         filteredConditionData.forEach(data => {
            msg = msg + `Please enter value of ${data.FieldName}  <br/>`
         });
        }
    ...
    function Multivalrender(props) {
        ...
        const handleChange = (e) => {
            ...
            if (props.data.SystemFlag === "Y") {
                props.data.FilterClause = props.data.ContextVariable;
            }
            setBaseObj({ ...baseObj });
    ...
    const renderSystemFlagStatus = (cellData) => {
        ...
        return (
            <div id>
                {cellData.data.SystemFlag === "Y" ? <i class="bi bi-exclamation-circle-fill" style={{color:'#03C04A', fontSize: '10pt'}}></i> : <></>}
            </div>
        );
    }
    ...
    const handleFilterClauseEdit = (props, baseObj) => {
        console.log("propspclause", props);
